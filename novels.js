export const novels = [
    {
        name: "Bofuri I Don’t Want to Get Hurt, so I’ll Max Out My Defense",
        volumes: 11,
        extras: null,
    },
    {
        name: "Campfire Cooking In Another World With My Absurd Skill",
        volumes: 14,
        extras: null,
    },
    {
        name: "Didn't I Say Make My Abilities Average In My Next Life",
        volumes: 17,
        extras: null,
    },
    {
        name: "Free Life Fantasy Online Immortal Princess",
        volumes: 5,
        extras: null,
    },
    {
        name: "Gun Gale Online",
        volumes: 13,
        extras: null,
    },
    {
        name: "Hikikomari Kyuuketsuki no Monmon",
        volumes: 6,
        extras: null,
    },
    {
        name: "I Parry Everything",
        volumes: 4,
        extras: null,
    },
    {
        name: "I Shall Survive Using Potions",
        volumes: 9,
        extras: null,
    },
    {
        name: "I'm In Love With The Villainess",
        volumes: 11,
        extras: [
            {
                type: "Series",
                name: "She's So Cheeky For A Commoner",
                volumes: 2,
            },
        ],
    },
    {
        name: "I've Been Killing Slimes For 300 Years And Became Max Level",
        volumes: 14,
        extras: null,
    },
    {
        name: "In the Land of Leadale",
        volumes: 8,
        extras: null,
    },
    {
        name: "Kuma Kuma Kuma Bear",
        volumes: 17,
        extras: [{ type: "Extra", volumes: ["Volume (11.5)"] }],
    },
    {
        name: "Kumo Desu Ga, Nani Ka",
        volumes: 16,
        extras: null,
    },
    {
        name: "Making Magic The Sweet Life of a Witch Who Knows an Infinite MP Loophole",
        volumes: 6,
        extras: null,
    },
    {
        name: "My Daughter Left the Nest and Returned an S-Rank Adventurer",
        volumes: 11,
        extras: null,
    },
    {
        name: "My Quiet Blacksmith Life in Another World",
        volumes: 8,
        extras: null,
    },
    {
        name: "Now I’m a Demon Lord! Happily Ever After with Monster Girls in My Dungeon",
        volumes: 7,
        extras: null,
    },
    {
        name: "Only the Villainous Lord Wields the Power to Level Up",
        volumes: 4,
        extras: null,
    },
    {
        name: "Overlord",
        volumes: 16,
        extras: [
            {
                type: "Extra",
                volumes: [
                    "The Vampire Princess Of The Lost Country",
                    "Movie 1 Bonus Pleiades Days",
                    "Blu-Ray 1 Special - Emissary Of The King",
                    "Blu-Ray 4 Special - Overlord Prologue (1st Half)",
                    "Blu-Ray 6 Special - Overlord Prologue (2nd Half)",
                ],
            },
        ],
    },
    {
        name: "Reincarnated as a Sword",
        volumes: 14,
        extras: null,
    },
    {
        name: "Reincarnated as an Apple This Forbidden Fruit Is Forever Unblemished!",
        volumes: 2,
        extras: null,
    },
    {
        name: "Reincarnator Executioner",
        volumes: 2,
        extras: null,
    },
    {
        name: "Saving 80,000 Gold in Another World for my Retirement",
        volumes: 3,
        extras: null,
    },
    {
        name: "She Professed Herself Pupil of the Wise Man",
        volumes: 10,
        extras: null,
    },
    {
        name: "Skeleton Knight In Another World",
        volumes: 10,
        extras: null,
    },
    {
        name: "Taking My Reincarnation One Step at a Time No One Told Me There Would Be Monsters!",
        volumes: 4,
        extras: null,
    },
    {
        name: "Tearmoon Empire",
        volumes: 11,
        extras: null,
    },
    {
        name: "That Time I Got Reincarnated As A Slime",
        volumes: 17,
        extras: null,
    },
    {
        name: "The Invincible Little Lady",
        volumes: 5,
        extras: null,
    },
    {
        name: "The Magical Revolution of the Reincarnated Princess and the Genius Young Lady",
        volumes: 6,
        extras: null,
    },
    {
        name: "The Villainess is the Heroine’s Biggest Fan",
        volumes: 1,
        extras: null,
    },
    {
        name: "The Weakest Tamer Began a Journey to Pick Up Trash",
        volumes: 5,
        extras: null,
    },
    {
        name: "Villainess Level 99 I May Be the Hidden Boss but I’m Not the Demon Lord",
        volumes: 3,
        extras: null,
    },
    {
        name: "VTuber Legend How I Went Viral after Forgetting to Turn Off My Stream",
        volumes: 6,
        extras: null,
    },
    {
        name: "Wandering Witch The Journey of Elaina",
        volumes: 12,
        extras: null,
    },
    {
        name: "Welcome to the Diner of the Exiled",
        volumes: 2,
        extras: null,
    },
    {
        name: "Youjo Senki",
        volumes: 12,
        extras: null,
    },
    {
        name: "ReMonster",
        volumes: 11,
        extras: null,
    },
];
